...
public class AIPath : AIBase {
	...
	public virtual void OnTargetReached () {
        ...
        target.gameObject.GetComponent<Cursor>().isSelecting = false;
        target.gameObject.GetComponent<Cursor>().unitIsMoving = false;
        RaycastHit2D hit = Physics2D.Raycast(this.GetComponent<Unit>().transform.position, new Vector3 (0.2f,0.2f,1));

        if (hit.collider.tag == "walkable" && hit.collider.transform != this.transform)
        {
              Mathf.Lerp(hit.collider.transform.position.x, this.transform.position.x, Time.deltaTime);
              Mathf.Lerp(hit.collider.transform.position.y,this.transform.position.y , Time.deltaTime);
        }
    }
